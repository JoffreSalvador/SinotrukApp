        ...
        children: [
          ...
          expensesAsync.when(
            ...
            data: (expenses) {
              ...
              final plateMap = {for (final v in vehicles) v.id: v.plate};
                ...
                children: [
                  ...
                  for (final e in expenses)
                    Dismissible(
                      ...
                      onDismissed: (_) async => ref.read(adminRepositoryProvider).deleteVehicleExpense(e.id),
                      child: ListTile(
                        leading: const Icon(Icons.receipt_long),
                        title: Text(e.detail),
                        subtitle: Text('${e.expenseDate} · ${plateMap[e.vehicleId] ?? e.vehicleId}'),
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDlg) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              DropdownButtonFormField<String>(initialValue: vehicleId, items: vehicles.map((v) => DropdownMenuItem(value: v.id, child: Text(v.plate))).toList(), onChanged: (v) => setDlg(() => vehicleId = v), decoration: const InputDecoration(labelText: 'Vehículo')),
              const SizedBox(height: 8),
